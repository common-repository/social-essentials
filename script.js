...
jQuery(document).ready(function(){
	...
	if (jQuery("#se_show_buttons").length) 
	{
		...
		var check_facebook = function(){
			if (jQuery("input[name=se_show_facebook]").is(":checked"))
				jQuery("input[name=se_show_fb_like],input[name=se_show_fb_share]").removeAttr('checked');
		}
		
		jQuery("input[name=se_show_facebook]").click(check_facebook);
		
		jQuery("input[name=se_show_fb_like],input[name=se_show_fb_share]").click(function(){
			if (jQuery(this).is(":checked"))
				jQuery("input[name=se_show_facebook]").removeAttr('checked');
		});
	...
	jQuery('#se_preview').click(function(){
		...
		var data = {
			...
			show_facebook: (jQuery('input[name=se_show_facebook]').is(':checked')) ? '1' : '0',
    ...
    jQuery('.se_per_day_filter').click(function(){
		jQuery('.se_per_day_filter').removeClass('se_selected');
		jQuery(this).addClass('se_selected');
		
		var data = {
			action: 'se_stats_per_day',
			type: (jQuery(this).attr('id') == 'posts_per_day_stats') ? 'post' : 'page',
            date: jQuery('#se_datepicker').val()
		};
		
		jQuery('#se_stats_per_day').html('<div class="se_preloader"></div>');
		
		// since 2.8 ajaxurl is always defined in the admin header and points to admin-ajax.php
		jQuery.post(ajaxurl, data, function(response) {			
			
			jQuery('#se_stats_per_day').html(response);
			
		});
	});
    
    jQuery('#se_datepicker').datepicker({
        dateFormat: "yy-mm-dd",
        maxDate: 0,
        minDate: -29,
        onSelect: function (dateText, ins){
            ...
            var data = {
    			action: 'se_stats_per_day',
    			type: (jQuery('.se_per_day_filter.se_selected').attr('id') == 'posts_per_day_stats') ? 'post' : 'page',
                date: jQuery('#se_datepicker').val()
    		};
    ...
    jQuery("#se_call_to_action_del_arrow").click(function(){
        jQuery('#se_call_to_action_block_current_arrow img#se_call_action_to_current_arrow').remove();
        jQuery('#se_call_action_to_arrow').val('');
    });
